...
public class WechatMessageUtils{
    ...
    public static String transferSendParse(String content){
        ...
        try{
            ...
            String  amount  = feedesc.getTextTrim();

            //转账备注
            Element pay_memo = wcpayinfo.element("pay_memo");
            String  memo     = pay_memo.getTextTrim();

            return MessageFormat.format("你发起了一笔{0}元的转账[{1}]", amount, memo);
        }catch(Exception e){
            throw new UtilsException(e.getMessage(), e);
        }
    }
    ...
    public static String transferDrawParse(String content){
        ...
        try{
            ...
            return MessageFormat.format("被领取了一笔{0}元的转账[{1}]", amount, memo);
        }catch(Exception e){
            throw new UtilsException(e.getMessage(), e);
        }
    }
    ...
    public static String getTransferType(String content){
        ...
        try{
            ...
            return paysubtype == null ? "" : paysubtype.getTextTrim();
        }catch(Exception e){
            throw new UtilsException(e.getMessage(), e);
        }
    }
    ...
    public static String luckyPackageSendParse(String content){
        ...
        try{
            ...
            return MessageFormat.format("你发起了一个红包[{0}]", senderTitle.getTextTrim());
        }catch(Exception e){
            throw new UtilsException(e.getMessage(), e);
        }
    }
    ...
    public static String systemParse(String content){
        ...
        try{
            ...
            //被领取红包
            pattern = Pattern.compile("<img src=\"SystemMessages_HongbaoIcon.png\"/>  [\\s\\S]*领取了你的<_wc_custom_link_ color=\"#FD9931\" href=\"weixin://weixinhongbao/opendetail\\?sendid=[0-9]*\">红包</_wc_custom_link_>");
            matcher = pattern.matcher(content);
            if(matcher.matches()){
                pattern = Pattern.compile("[^(<img src=\"SystemMessages_HongbaoIcon.png\"/>)][\\s\\S]*领取了你的");
                matcher = pattern.matcher(content);
                if(matcher.find()){
                    return matcher.group() + "红包";
                }
    ...
    public static Boolean isPassFriend(String content){
        //好友添加
        Pattern pattern = Pattern.compile("你已添加了[\\s\\S]*，现在可以开始聊天了。");
        Matcher matcher = pattern.matcher(content);
